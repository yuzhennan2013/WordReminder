...
public class StringUtils {
	...
	public static String[] split(String input, String separator) {
		...
		if (input.equals("")) return EMPTY_STRING;
		if (separator == null || "".equals(separator)) return new String[] { input };
		...
		if (lastPos < input.length()) list.add(input.substring(lastPos));
	...
	public static String trimTail(StringBuffer sb, char tail) {
		if (sb.length() > 0 && sb.charAt(sb.length() - 1) == tail) sb
	...
	public static Integer[] splitInt(String input, String separator) {
		...
		if (separator == null || "".equals(separator)) return null;
		...
		if (lastPos < input.length()) list.add(Integer.parseInt(input
				.substring(lastPos)));
